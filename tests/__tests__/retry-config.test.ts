...
import { DEFAULT_RETRY_CONFIG, GoogleServiceRetryConfig } from '../../src/services/base/google-service';
...
describe('Retry Configuration TDD Tests', () => {
  ...
  describe('Configuration Validation & Defaults', () => {
    test('should have default retriable codes', () => {
      ...
      expect(DEFAULT_RETRY_CONFIG.retriableCodes).toEqual([429, 500, 502, 503, 504]);
    });
    ...
    test('should validate positive numbers for retry parameters', () => {
      ...
      expect(() => {
        if (config.maxAttempts <= 0) throw new Error('maxAttempts must be positive');
        if (config.initialDelayMs <= 0) throw new Error('initialDelayMs must be positive');
        if (config.maxDelayMs <= 0) throw new Error('maxDelayMs must be positive');
        if (config.backoffMultiplier <= 0) throw new Error('backoffMultiplier must be positive');
        if (config.jitterFactor < 0 || config.jitterFactor > 1) throw new Error('jitterFactor must be between 0 and 1');
      }).toThrow();
    });

    test('should filter out non-retriable HTTP codes', () => {
      const nonRetriableCodes = [100, 200, 201, 204, 300, 301, 400, 401, 403, 404];
      const allCodes = [...nonRetriableCodes, 429, 500, 502, 503, 504];
      
      const filtered = allCodes.filter(code => [429, 500, 502, 503, 504].includes(code));
    ...
    test('should normalize baseDelayMs alias to initialDelayMs', () => {
      ...
      delete (normalized as any).baseDelayMs;
  ...
  describe('Environment Variable Loading', () => {
    ...
    test('should load GOOGLE_RETRY_RETRIABLE_CODES from environment', () => {
      ...
      const config = loadConfig();
      expect(config).toHaveProperty('GOOGLE_RETRY_RETRIABLE_CODES', [429, 500, 502]);
    });
  ...
  describe('HTTP Status Code Logic', () => {
    ...
    test('should prioritize HTTP codes over error.isRetryable()', () => {
      ...
      const shouldRetry = retriableCodes.includes(error.statusCode) || error.isRetryable();
  ...
  describe('Enhanced Logging', () => {
    test('should format attempt numbers clearly', () => {
      const formatAttempt = (current: number, max: number) => `Attempt ${current}/${max}`;
    ...
    test('should format next delay information', () => {
      const formatRetryDelay = (delayMs: number) => `Retrying in ${delayMs}ms`;
    ...
    test('should format retry reasons', () => {
      const formatRetryReason = (type: string, value: number | string) => `${type}: ${value}`;
      
      expect(formatRetryReason('retriable_http_status', 500)).toBe('retriable_http_status: 500');
      expect(formatRetryReason('error_not_retryable', 'Invalid credentials')).toBe('error_not_retryable: Invalid credentials');
    });
  });
  ...
  describe('Error System Integration', () => {
    ...
    test('should maintain compatibility with existing error hierarchy', () => {
      ...
      class GoogleServiceError extends Error {
        ...
        isRetryable() {
          return this.code.includes('RATE_LIMIT') || this.code.includes('SERVER_ERROR');
        }
      }

      const rateLimitError = new GoogleServiceError('Rate limit', 'RATE_LIMIT_ERROR');
