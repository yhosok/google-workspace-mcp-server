...
export class SheetsTools {
  ...
  constructor(authService: AuthService, sheetsService: SheetsService, logger?: Logger) {
   ...
   */
  async sheetsRead(spreadsheetId: string, range: string): Promise<Result<SheetsReadResult, GoogleWorkspaceError>> {
    ...
    const validationError = this.validateParameters(spreadsheetId, range, 'read');
    if (validationError) {
    ...
    try {
      ...
      const sheetDataResult = await this.sheetsService.readRange(spreadsheetId, range);
   ...
   */
  async sheetsWrite(spreadsheetId: string, range: string, values: string[][]): Promise<Result<SheetsWriteResult, GoogleWorkspaceError>> {
    ...
    // Parameter validation
    const validationError = this.validateParameters(spreadsheetId, range, 'write', values);
    if (validationError) {
    ...
    try {
      ...
      const writeResult = await this.sheetsService.writeRange(spreadsheetId, range, values);
   ...
   */
  async sheetsAppend(spreadsheetId: string, range: string, values: string[][]): Promise<Result<SheetsAppendResult, GoogleWorkspaceError>> {
    ...
    // Parameter validation
    const validationError = this.validateParameters(spreadsheetId, range, 'append', values);
    if (validationError) {
    ...
    try {
      ...
      const appendResult = await this.sheetsService.appendData(spreadsheetId, range, values);
  ...
  async getHealthStatus(): Promise<Result<{
    ...
    try {
      ...
      const sheetsHealthy = sheetsHealthResult.isOk() && sheetsHealthResult.value;
