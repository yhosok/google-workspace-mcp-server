...
describe('SheetsService', () => {
  ...
  describe('Spreadsheet Information Retrieval', () => {
    ...
    test('should return error for invalid spreadsheet ID', async () => {
      ...
      const error = new Error('Invalid spreadsheet ID') as Error & {
        code: number;
      };
      error.code = 400;
    ...
    test('should return error for empty spreadsheet ID', async () => {
      ...
      if (result.isErr()) {
        expect(result.error.message).toMatch(
          /Invalid range specified|Spreadsheet ID cannot be empty/
        );
      }
    });
  ...
  describe('Range Reading Operations', () => {
    ...
    test('should handle invalid range format', async () => {
      ...
      for (const range of invalidRanges) {
        ...
        if (result.isErr()) {
          expect(result.error.message).toMatch(
            /Invalid range format|Invalid range specified|Range is required|Range cannot be empty/
          );
        }
      }
    });
  ...
  describe('Range Writing Operations', () => {
    ...
    test('should validate range and values dimensions', async () => {
      ...
      if (result.isErr()) {
        expect(result.error.message).toMatch(
          /Range and values dimensions do not match|Invalid range specified/
        );
      }
    });
  ...
  describe('Data Appending Operations', () => {
    ...
    test('should handle empty values in append', async () => {
      ...
      if (result.isErr()) {
        expect(result.error.message).toMatch(
          /Values cannot be empty for append operation|Invalid range specified/
        );
      }
    });
  ...
  describe('Error Handling Scenarios', () => {
    ...
    test('should handle network errors', async () => {
      // ネットワークエラーのシミュレーション
      (
        mockDriveApi.files.list as jest.MockedFunction<
          typeof mockDriveApi.files.list
        >
    ...
    test('should handle API rate limiting', async () => {
      // レート制限エラーのシミュレーション
      (
        mockSheetsApi.spreadsheets.values.get as jest.MockedFunction<
          typeof mockSheetsApi.spreadsheets.values.get
        >
    ...
    test('should handle quota exceeded errors', async () => {
      // API割当超過エラーのシミュレーション
      (
        mockSheetsApi.spreadsheets.values.update as jest.MockedFunction<
          typeof mockSheetsApi.spreadsheets.values.update
        >
      ).mockRejectedValue(new Error('Quota exceeded'));
      const result = await sheetsService.writeRange('sheet-id', 'Sheet1!A1', [
        ['data'],
      ]);
      expect(result.isErr()).toBe(true);
    });
  ...
  describe('AuthService Integration', () => {
    ...
    test('should use authenticated client from AuthService', async () => {
      ...
      mockAuthService.getAuthClient.mockResolvedValue(ok(customAuth));
  ...
  describe('Concurrent Initialization Prevention', () => {
    ...
    test('should handle concurrent calls to ensureInitialized through different methods', async () => {
      ...
      (
        mockSheetsApi.spreadsheets.get as jest.MockedFunction<
          typeof mockSheetsApi.spreadsheets.get
        >
      ).mockResolvedValue({
        data: {
      ...
      mockAuthService.getAuthClient = jest.fn().mockImplementation(() => {
        ...
        if (callCount === 1) {
          ...
          return Promise.resolve(
            err(new GoogleAuthError('First attempt failed'))
          );
        } else {
  ...
  describe('Fast Path Performance Optimization', () => {
    ...
    test('should have fast execution time when already initialized', async () => {
      ...
      const promises = Array(10)
        .fill(null)
        .map(() => sheetsService.listSpreadsheets());
